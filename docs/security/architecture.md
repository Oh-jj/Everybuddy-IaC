...
```
Ingress:
  - 22/tcp  ← Bastion SG 전용 (SSH ProxyJump)
  - 8080/tcp ← ALB SG 전용 (Spring Boot)
  - 8080/tcp ← Monitoring SG (Actuator 메트릭)
  - 9100/tcp ← Monitoring SG (Node Exporter)
  - 3100/tcp → Monitoring SG (Loki 로그 푸시, outbound 규칙)
Egress:
  - 전체 허용 (NAT GW 경유 아웃바운드)
```

### RDS SG
```
Ingress:
  - 3306/tcp ← Private Backend SG
  - 3306/tcp ← Backend SG
  - 3306/tcp ← Bastion SG (SSH 터널 경유 개발자 접근)
Egress:
  - 전체 허용
```
...
```
Ingress:
  - 22/tcp ← 전체 (GitHub Actions CI/CD ProxyJump 필요)
Egress:
  - 전체 허용
```
...
```
Ingress:
  - 없음 (Lambda는 이벤트 트리거 기반, 인바운드 불필요)
Egress:
  - 전체 허용 (VPC 내 Prometheus + NAT GW 경유 Bedrock/Slack)
```
...
- ALB Listener 80 → 443 리디렉션 설정
- 클라이언트 ↔ ALB 구간 TLS 1.2+ 적용

### 내부 통신
- ALB ↔ Private Backend: HTTP (VPC 내부, 네트워크 격리로 대체)
- Private Backend ↔ RDS: 평문 (Private Subnet 내부 통신)
...
### SSM Parameter Store
Slack Webhook URL 등 민감 정보는  
SSM Parameter Store `SecureString` 타입으로 저장 (KMS 암호화).
...
| 항목 | 관리 방식 |
|------|-----------|
| RDS 비밀번호 | 환경변수(`TF_VAR_db_password`)로 주입, tfvars 미포함 |
...
| SSM SecureString으로 Webhook 관리 | 코드/환경변수에 민감 정보 노출 방지 |
